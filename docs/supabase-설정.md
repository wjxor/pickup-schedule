...
프로젝트 생성 화면의 Security 항목이다.

| 설정 | 값 | 이유 |
|---|---|---|
| Enable Data API | **켬 (필수)** | supabase-js 의 `.from(...)` 호출이 전부 이 API를 통한다. 끄면 앱이 아예 동작하지 않는다 |
| Automatically expose new tables | **켬** | 끄면 마이그레이션으로 만든 `games` / `events` 가 API에 보이지 않아 테이블마다 수동 등록이 필요하다 |
| Enable automatic RLS | **켬** | 새 테이블에 RLS를 자동으로 걸어준다. 데이터 유출 사고 대부분이 "RLS 켜는 걸 잊은 테이블"에서 나온다 |

**"노출(expose)"은 "공개(public)"가 아니다.**
노출은 API 문 앞까지 요청이 도달할 수 있다는 뜻일 뿐이고,
실제로 데이터를 볼 수 있는지는 RLS 정책이 따로 결정한다.
그래서 두 번째를 켜도 세 번째가 켜져 있으면 안전하다.

RLS가 켜진 테이블은 **정책이 없는 한 아무도 읽지 못한다(기본 거부).**
이 프로젝트의 마이그레이션은 `games` / `events` 에 읽기 허용 정책을 함께 넣으므로
화면 조회는 정상 동작하고, 쓰기는 RLS를 우회하는 `service_role` 키로만 가능하다.
...
1. 왼쪽 메뉴 맨 아래 **Project Settings** → **API Keys** 로 간다.
   ...
   > **키 이름이 문서와 다르게 보일 수 있다.**
   > Supabase가 API 키 명칭을 바꾸는 중이라 아래처럼 표시될 수 있는데, 역할은 같다.
   >
   > | 이 문서의 이름 | 새 이름 |
   > |---|---|
   ...
   >
   > 공개용 키를 `NEXT_PUBLIC_SUPABASE_ANON_KEY` 에,
   > 비밀 키를 `SUPABASE_SERVICE_ROLE_KEY` 에 넣으면 된다.
   > 코드는 키 문자열을 그대로 전달할 뿐이라 어느 형식이든 동작한다.
